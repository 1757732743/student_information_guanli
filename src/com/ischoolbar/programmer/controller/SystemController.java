...
public class SystemController {
	...
	public Map<String, String> login(
		...
		//开始判断是进入哪一个页面哦，管理员的，还是学生的。从数据库中去查找用户,这个地方就调用了dao层里面的数据库操作
		//下面我想利用不同dao层来z
		if(type == 1){
			//管理员，注意这里有一个实体对对象
		...
		if(type == 2){
			//学生,改为new
			Student_new student = studentService.findByUserName(username);
			if(student == null){
				ret.put("type", "error");
				ret.put("msg", "不存在该学生!");
				return ret;
	...
	public void getCpacha(HttpServletRequest request,
		...
		//下面这个是session绘画，把它传给sessoion进行保存
