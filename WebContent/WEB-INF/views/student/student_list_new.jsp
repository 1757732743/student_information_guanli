                ...
                <form class="layui-form layui-form-pane" action="" lay-filter="searchForm">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label">姓名</label>
                            <div class="layui-input-inline">
                                <input type="text" name="sname" autocomplete="off" class="layui-input" readonly=${user.sname} value=${user.sname}>
                            </div>
                        </div>
                        <div class="layui-inline">
                            <button type="submit" class="layui-btn layui-btn-primary"  lay-submit lay-filter="data-search-btn"><i class="layui-icon"></i> 点击搜索个人信息 </button>
                        </div>
...
<script>
    layui.use(['form', 'table','laydate', "upload", "element", "layer"], function () {
        ...
        var uploadInst = upload.render({
            elem: "#test1",
            url: "/updata/updateImg.do", //此处用的是第三方的 http 请求演示，实际使用时改成您自己的上传接口即可。
            before: function (obj) {
                //预读本地文件示例，不支持ie8
...
//这个就是学生信息管理的主界面
        $(function () {
            //获取所有的班级信息
            $.getJSON({
                url: 'clazz_new/queryAllClazzs.do',
                success: function (data) {
                    $("#search_cid").html();
                    $.each(data,function (i,n) {
                        $("#search_cid")
                            .append("<option value=\""+n.name+"\">"+n.name+"</option>")
                    });
        ...
        //加载数据表格,如果是管理员就全部显示，但是我这里就我不想展示出表格，我就仅展示出列来就行了
        <c:if test="${userType == 2}">
         table.render({
             elem: '#currentTableId',
             //url: 'student_new/queryStudents.do',
             toolbar: '#toolbarDemo',
       ...
       <c:if test="${userType == 1}">
        ...
        form.on('submit(data-search-btn)', function (data) {
            ...
            table.reload('currentTableId', {
                url: 'student_new/searchStudents.do',
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                layer.open({
                    ...
                    success: function () {  //弹出框成功回调
                        ...
                        form.val("editForm", {
                            ...
                            "sstatus":'正常',
                            "sremark":'无',
                            "idcard":'51072319970716003',
                            "phone":'13912345678',
                            "address":'四川省泸州市',
                            "entime":'2019-09-01',
                            "pswd":'123456',
                            "pic": 'http://localhost:8080/StudentManagerSSM/photo/1.jpg'
                        });
                        $("#demo1").attr("src", "http://localhost:8080/StudentManagerSSM/photo/1.jpg");
                    },
                ...
                layer.confirm('确定要删除选中行吗？', function(index){
                    ...
                    //向服务器请求
                    $.getJSON({
                        url: 'student_new/deleteStudents.do',
                ...
                layer.open({
                    ...
                    area: ['90%', '90%'],
                    btn: ['确定'],
                    content: 'manager_student_information.jsp?sid=' + data[0].sid,
                    yes: function (index, layero) { //确认的回调
                        layer.close(index); //关闭弹出框
                    }
                })
                ...
                console.log("nihao");
                $.getJSON({
                    url: 'student_new/printStudentInformation.do',
                    success:function (res) {
                        if(res.code === 200) {
                            // 输出提示信息
            ...
            if (obj.event === 'edit') { //监听编辑按钮
                var index = layer.open({
                    ...
                    yes: function () {  //确认回调
                        ...
                        $.getJSON({
                            url: 'student_new/updateStudent.do',
